function [result_table, best_norm, best_E, best_G] = test_case_parallel(method,X,Y,E_org,train_delete_rate, test_delete_rate,variable_set_1,variable_set_2,variable_set_3)
    ...
    for i=1:length(variable_set_1)
        ...
        for j=1:length(variable_set_2)
            ...
            for k=1:length(variable_set_3)
                ...
                start_time = cputime;
                if strcmp(method, 'AFA_ALDM')           
                    [G_new, E_new] = AFA_ALDM_parallel(X_shuffled, Y, E, variable_1, variable_2, maxIter, eps, variable_3, mult);
